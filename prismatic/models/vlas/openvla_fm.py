...
class OpenVLAFlowMatching(nn.Module):
    def __init__(self, vla_path, torch_dtype, quantization_config, low_cpu_mem_usage=True,
        ...
        self.action_expert = DeepActionExpert(
            hidden_dim=self.vla.config.text_config.hidden_size,
            num_heads=self.vla.config.text_config.num_attention_heads,
            num_layers=3
        )
    ...
    def forward(
        ...
        flow = self.action_expert(
            ...
            vlm_hidden_states=outputs.hidden_states[-3:],
            robot_state=proprio
        )
        return outputs, flow
...
class DeepActionExpert(nn.Module):
    def __init__(self, hidden_dim, num_heads, num_layers):
        ...
        self.initial_action_transform = nn.Linear(7, hidden_dim)
...
class ActionCrossAttention(nn.Module):
    def __init__(self, hidden_dim, num_heads):
        ...
        self.k_proj_robot = nn.Linear(8, hidden_dim)
        self.v_proj_robot = nn.Linear(8, hidden_dim)
